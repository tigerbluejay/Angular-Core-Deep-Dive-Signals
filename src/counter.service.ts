...
/* Here we create a data service so we can create a signal which is not dependent on any 
particular Component.
We inject the service in the AppComponent constructor and we can invoke the Service in 
the template.

The signal is private within the service.
The service provides an api (increment())
This keeps our signal code maintainable
This is when you have a signal you want to share the signal between unrelated components
*/
...
// In this way, we replace OnChanges hook log, with a log as a side effect of a signal
*/

/* SIGNAL INPUT OPTIONS */

/* We can define an input to be required just as we would a traditional input decorated 
variable. To do so we type
// course = input.required<Course>(); 
// this means that the template (html) must provide an initial value
// <course-card [course] = "course"></course-card>

We can define an alias to replace the name of the member variable when we call the input 
signal. We can also transform the original value of the input into something else
